function [inputImages, inputSignals, valid, imageSizes, imgFeatures] = filterImages(inputImages, inputSignals, varargin)
    ...
    valid = valid(:)';
    if options.modifyInputImage==1
        [imgStats] = computeImageFeatures(inputImages,'thresholdImages',0,'featureList',options.featureList);
    else
        [imgStats] = computeImageFeatures(inputImagesCopy,'thresholdImages',0,'featureList',options.featureList);
    end
    ...
    if ~isempty(inputSignals)
        ...
        % Slope ratio
        [peakOutputStat] = computePeakStatistics(inputSignals,'waitbarOn',options.waitbarOn);
