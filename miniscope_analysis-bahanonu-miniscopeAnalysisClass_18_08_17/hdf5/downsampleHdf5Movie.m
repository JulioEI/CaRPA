	...
	try
		...
		for currentSubset=1:nSubsets
			...
			if ~isempty(options.saveFolderTwo)
				...
				% downsample section of the movie, keep in memory
				downsampleMovieNested('downsampleDimension', 'space','downsampleFactor',options.downsampleFactorTwo);
				% save the movie
				if currentSubset==1
					createHdf5File(options.newFilenameTwo, options.outputDatasetName, inputMovie);
				else
