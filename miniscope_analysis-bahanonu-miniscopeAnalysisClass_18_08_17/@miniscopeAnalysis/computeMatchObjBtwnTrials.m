function obj = computeMatchObjBtwnTrials(obj)
	...
	% end
	scnsize = get(0,'ScreenSize');
	userDefaults = {'1','5','0.5'};
	usrIdxChoice = inputdlg({'number of rounds to register images','distance to match (px)','image threshold value (0 to 1)'},'options',1,userDefaults);
	...
	for thisSubjectStr=subjectList
		try
			...
			alignmentStruct = matchObjBtwnTrials(rawImages,'inputSignals',rawSignals,'trialToAlign',trialToAlign,'additionalAlignmentImages',[],'nCorrections',nCorrections,'maxDistance',maxDistance,'threshold',imageThreshold);
