function [cnmfAnalysisOutput] = computeCnmfSignalExtraction(inputMovie,numExpectedComponents,varargin)
	% wrapper function for CNMF, update for most recent versions.
	% building off of demo_script.m in CNMF github repo
	% most recent commit tested on: https://github.com/epnev/ca_source_extraction/commit/08929818138f99d9026ff22cd296572b38058a38.
	% biafra ahanonu
	% started: 2016.01.20
	% inputs
	...
	options.ssub = 2;  % Spatial down-sampling factor (scalar >= 1)  1
	...
	try
		...
		Cn = reshape(P.sn,d1,d2);reshape(P.sn,d1,d2);
	    ...
	    [C,f,P,S] = update_temporal_components(Yr,A,b,Cin,fin,P,cnmfOptions);

	    %% merge found components
	    [Am,Cm,K_m,merged_ROIs,P,Sm] = merge_components(Yr,A,b,C,f,P,S,cnmfOptions);
	    ...
	    [C2,f2,P,S2] = update_temporal_components(Yr,A2,b2,Cm,f,P,cnmfOptions);
	    ...
	    C_df_or_background = C_df_or(end,:);
		...
		cnmfAnalysisOutput.extractedSignalsBackground = C_df_or_background;
