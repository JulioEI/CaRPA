function [exitSignal ostruct] = playMovie(inputMovie, varargin)
	...
	supTitleStr = [10,10,10,'e:exit    q:exit all    g:goto frame    p:pause/play    r:rewind    f:forward    j:contrast    l:label    i:imagej',10,'c:crop    d:\DeltaF/F    a:downsample    n:normalize    m:\Deltacolormap    +:speed    -:slow    ]:+1 frame    [:-1 frame    dims: ' num2str(size(inputMovie)),'    ',strrep(options.extraTitleText,'\','/'),10,repmat(' ',1,7),10,10];
		 ...
		 %               'alimmode','manual');
		    ...
		    if ~isempty(options.primaryPoint)
		    	if length(options.primaryPoint(:,1))==1
		    		plot(options.primaryPoint(:,1), options.primaryPoint(:,2), 'ko', 'Markersize', 10)
		    	else
