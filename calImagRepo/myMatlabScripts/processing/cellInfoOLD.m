...
%             obj.validCells(randVect(ceil(obj.cellN/3):2*floor(obj.cellN/3)))=1;
        ...
        function gui(obj)
            ...
            if ~res 
               ...
               switch indx
                   case 1
                       obj.manualSort('tresholds',{'getOverlap','>=.4','getglobalSNR','>1'});
                   case 2
...
%                 imagesc(obj.cellMask)
            ...
            if obj.parseInput({varargin,'weighted',1})
...
%                 x(x>.5) = 2./(1+exp(-10*(x(x>.5))+5));
        ...
        function [scores] = predictClassifier(obj,varargin)
            cellList = obj.parseInput({varargin,'cellList',1:length(obj.signalPeakIdx)});
            probTresh = obj.parseInput({varargin,'probTresh',.9});
            ...
            scores = zeros([1,length(predBasic)]);
            scores(basicGoodIdx) = predProb;
        ...
        function setTresholds(obj,varargin)
            %Accepts input like obj.setTresholds('tresholds',{'getOverlap','>=0.6','getCellShapePropieties.Eccentricity','<=0.9','getCellShapePropieties.Solidity','>0.8'},'eventPercent',.2)
        ...
        function manualSort(obj,varargin)
            ...
            while stay
                ...
                if ambiguous
                    cellListAmbiguous = intersect(cellList,find(obj.validCells==3));
