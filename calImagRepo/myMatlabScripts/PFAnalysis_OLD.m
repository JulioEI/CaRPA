classdef PFAnalysis < singleAnimalAnalysis
    ...
    methods
        ...
        function PFinf = computeCellInformationSession(obj,varargin)
            ...
            for k = 1:nSessions
                PFinf{k} = PFAnalysis.computeCellinformation(r{k}, pos{k}, param);    
            end
...
%             r = r(framesDirection,:);
            ...
            %Integrate in time
            [rTB,xTB] = PFAnalysis.integrateInTime(r,x,dT,dtCamera);
...
%             figure;plot(linspace(5,length(x)+5,length(xTB)),PFAnalysis.binPosition(xTB,binN),'-o');hold on;plot(PFAnalysis.binPosition(x,binN),'-*');plot(repmat(1:length(x),[20,1])',(ones([20,length(x)]).*linspace(1,20,20)')','k--')
            
            %Bin positions
            [xTB,binCounts] = PFAnalysis.binPosition(xTB,binN);
