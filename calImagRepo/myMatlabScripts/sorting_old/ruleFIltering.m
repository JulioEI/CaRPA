...
%dataFeatures = nan([2,size(emAnalysisOutput.cellImages,3)]);

for j = 1:size(emAnalysisOutput.cellImages,3)
    %disp(['CELL ',num2str(j)])
    cellShape = emAnalysisOutput.cellImages(:,:,j);
    ...
    windowTracesAvg = mean(windowTraces);
